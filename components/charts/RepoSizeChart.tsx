...
export default function RepoSizeChart({ data, isLoading }: Props) {
  if (isLoading) {
    return (
  ...
  const options = {
    ...
    plugins: {
      ...
      tooltip: {
        ...
        displayColors: false,
        callbacks: {
          label: (ctx: any) => `Size: ${ctx.parsed.y} KB`,
        },
      },
    },
  };
  ...
  return (
    <div className="bg-white rounded-xl p-6 shadow-lg shadow-purple-100/50 border border-purple-100/20">
      <h3 className="text-lg font-semibold mb-4">Repository Size</h3>
      <div className="h-64">
        <Line
          data={chartData}
          options={options}
          role="img"
          aria-label="Area chart showing repository size over time"
        />
      </div>
    </div>
  );
